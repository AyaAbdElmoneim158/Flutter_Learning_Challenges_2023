...
import 'package:app/Usama%20Elgendy/big_project_movies/movies/presentation/screens/movie_details_screen/dummy.dart';
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: BlocBuilder<MovieDetailsBloc, MovieDetailsState>(
          builder: (context, state) {
        debugPrint(state.movieDetails!.backdropPath.toString());
        debugPrint(state.movieDetails!.genres.toString());
        debugPrint(state.movieDetails!.id.toString());
        debugPrint(state.movieDetails!.overview.toString());
        debugPrint(state.movieDetails!.releaseDate.toString());
        debugPrint(state.movieDetails!.runtime.toString());
        debugPrint(state.movieDetails!.title.toString());
        debugPrint(state.movieDetails!.voteAverage.toString());
        return Scaffold(
          body: Center(child: Text(state.movieDetails!.title.toString())),
          // MovieDetailContent(),
        );
      }),
    );
...
/*class MovieDetailContent extends StatelessWidget {
                ...
                background: FadeIn(
                  ...
                  child: ShaderMask(
                    ...
                    child: CachedNetworkImage(
                      width: MediaQuery.of(context).size.width,
                      imageUrl: ApiConstants.imageUrl(
                          state.movieDetails!.backdropPath.toString()),
                      fit: BoxFit.cover,
                    ),
                  ),
                ),
              ...
              child: FadeInUp(
                ...
                child: Padding(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(state.movieDetails!.title.toString(),
                          style: GoogleFonts.poppins(
                            fontSize: 23,
                            fontWeight: FontWeight.w700,
                            letterSpacing: 1.2,
                      ...
                      const SizedBox(height: 20.0),
                      Text(
                        state.movieDetails!.overview.toString(),
                        style: const TextStyle(
                          fontSize: 14.0,
                          fontWeight: FontWeight.w400,
                          letterSpacing: 1.2,
  ...
  Widget _showRecommendations() {
    return BlocBuilder<MovieDetailsBloc, MovieDetailsState>(
      builder: (context, state) {
        switch (state.requestStateMovieDetails) {
          case RequestState.loading:
